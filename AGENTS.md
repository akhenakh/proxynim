...
- **`injectModelSpecificParams`**: Patches the request JSON with `chat_template_kwargs` or `reasoning_effort` for specific NVIDIA NIM models that require them (glm-5.1, kimi-k2, nemotron-3, nemotron-3-super, deepseek-v4-pro, deepseek-v4-flash, mistral-medium, mistral-small-4, minimax-m2.7). Checks by `strings.Contains` on model name — not exact match.
